...
class FamilyModel {
    ...
    init() {
        ...
        self.members.append(FamilyMember(id: 1, fatherId: 0, motherId: 0, gender: false, isAffected: true))
        self.members.append(FamilyMember(id: 2, fatherId: 0, motherId: 0, gender: true, isAffected: false))
        self.members.append(FamilyMember(id: 3, fatherId: 0, motherId: 0, gender: false, isAffected: false))
        self.members.append(FamilyMember(id: 4, fatherId: 1, motherId: 2, gender: true, isAffected: true))
        self.members.append(FamilyMember(id: 5, fatherId: 3, motherId: 4, gender: true, isAffected: true))
        self.members.append(FamilyMember(id: 6, fatherId: 3, motherId: 4, gender: false, isAffected: false))
    }
    
    func drawPedigreeOntoView(view: UIView) {
        connectMembers(view: view)
        placeMembers(view: view)
    }
    
    private func placeMembers(view: UIView) {
        let v_m1 = members[0].getView();
        v_m1.frame = v_m1.frame.offsetBy(dx: 40, dy: 60)
        view.addSubview(v_m1)
        
        let v_m2 = members[1].getView();
        v_m2.frame = v_m2.frame.offsetBy(dx: 170, dy: 60)
        view.addSubview(v_m2)
        
        let v_m3 = members[2].getView();
        v_m3.frame = v_m3.frame.offsetBy(dx: 235, dy: 150)
        view.addSubview(v_m3)
        
        let v_m4 = members[3].getView();
        v_m4.frame = v_m4.frame.offsetBy(dx: 105, dy: 150)
        view.addSubview(v_m4)
        
        let v_m5 = members[4].getView();
        v_m5.frame = v_m5.frame.offsetBy(dx: 105, dy: 240)
        view.addSubview(v_m5)
        
        let v_m6 = members[5].getView();
        v_m6.frame = v_m6.frame.offsetBy(dx: 235, dy: 240)
        view.addSubview(v_m6)
    }
    
    private func connectMembers(view: UIView) {
