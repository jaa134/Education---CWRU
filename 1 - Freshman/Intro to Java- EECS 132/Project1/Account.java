...
public class Account  {
    ...
    /** Exta Credit enum. Will set the values of months to have three lettered variable names. This is my attempt of the enum, but it didn't exactly workout.
      * Changing all of my methods around to work according to the enum proved too difficult so heres the attempt atleast. Everything else that was modified
      * for the enum was either removed or changed back to the original.
  public enum Month {
  ...
  public Account(double lineFee, double rate, double taxRate, double interestRate, int expectedMonthlyUsage) {
    ...
    this.pastYearUsage = 12 * expectedMonthlyUsage;
 ...
 /** Second constructor for the account class. This constructor sets the line fee, the account rate, the tax rate, the interest rate, the expected monthly usage, the account's cusotmer,
  ...
  public int getPastYearUsage() {
    return (this.expectedMonthlyUsage * 12);
  }
  ...
  public void endOfMonthProcessing() {
    ...
    if(accountOpen == true) {
      ...
      this.balance = this.balance + ((1 + this.taxRate) * this.lineFee) + ((1 + this.taxRate) * (this.currentMonthlyUsage * (this.rate)));
    if(amountPaidThisMonth < monthBill) {
      this.balance = balance + (interestRate * (monthBill - amountPaidThisMonth));
    }
    ...
    if(monthlyPlan == true) {
      if(monthBill >= amountPaidThisMonth) {
        this.monthBill = interestRate * (monthBill - amountPaidThisMonth);
      }
      else 
        monthBill = amountPaidThisMonth - monthBill;
      monthBill = monthBill + ((lineFee + rate * currentMonthlyUsage) * taxRate);
    } 
